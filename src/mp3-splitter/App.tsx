...
export const App: React.FC = () => {
  ...
  const handleSplit = async () => {
    ...
    await runAction('Loading FFmpeg...', async () => {
  ...
  return (
    <PageLayout>
      <Panel title="MP3 Splitter">
        ...
        />
        <Button onClick={handleSplit} loading={isLoading} id="split-btn">Split MP3</Button>
        ...
        {outputs.length > 0 && (
          <ul id="output-list" style={{ marginTop: '1em' }}>
            {outputs.map(out => (
              <li key={out.name}>
                <a href={out.url} download={out.name}>Download {out.name}</a>
              </li>
            ))}
          </ul>
        )}
