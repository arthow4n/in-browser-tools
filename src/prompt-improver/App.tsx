...
export const App: React.FC = () => {
  ...
  const { state, set, undo, redo, canUndo, canRedo } = useUndoRedo<PromptState>(
    loadInitialState(),
  ...
  useEffect(() => {
    ...
    const baseWords =
      state.originalPrompt.split(' ').length +
      state.intention.split(' ').length +
      state.howToImprove.split(' ').length;
      ...
      async () => {
        ...
        while (true) {
          ...
          if (event.type === 'cost_estimation') {
            ...
            setLogs((prev) => [
              ...prev,
              { type: event.type, message: event.message, data: event.data },
            ]);
          }
        }
      ...
      <Panel title="Configuration">
        ...
        <div className="input-group">
          ...
          <select
            ...
            onChange={(e) => updateState('promptType', e.target.value)}
          >
        ...
        <div style={{ marginTop: '10px', fontSize: '0.9em', color: '#666' }}>
          ...
          <br />
          API Calls: <span id="api-calls-est">{apiCallsEst}</span>
          <br />
          Token Usage roughly correlates to words:{' '}
          <span id="words-est">~{wordsEst}</span> total words processed across
          all calls.
        </div>

        <div style={{ marginTop: '20px' }}>
          <Button onClick={handleStart} loading={isLoading} id="start-btn">
            Start Prompt Improver
          </Button>
          <span
            id="status-text"
            className="status"
            style={{ color: isError ? 'red' : 'green', marginLeft: '10px' }}
          >
